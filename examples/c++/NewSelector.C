...
void NewSelector::Begin(TTree *tree)
{
   ...
   printf("Creating Histograms with process option: %s\n",option.Data());

   h1 = new TH1F("h1","CaloJet Energy",200,0.,600.);

   //load data files
   //TChain chain("Events");
   //chain.Add("../rootfiles/tower_jet_pool_howt_3.0.root");   
}
...
Bool_t NewSelector::Process(Long64_t entry)
{
  ...
  //b_CaloJet_mcone5_obj_->GetEntry(entry);
  b_CaloJet_mcone5_obj_data_e->GetEntry(entry);
  return kTRUE;
}
